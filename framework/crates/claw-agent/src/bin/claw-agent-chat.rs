...
async fn run() -> Result<()> {
    ...
    claw_log::init_logger(
        LevelFilter::Trace,
        LogOutput::File(crate_dir.join("../claw-agent/logger.log")),
    )?;
    init_chat_tracing(&crate_dir.join("../claw-agent/simulator.log"))?;
    let env_path = crate_dir.join("../claw-agent/.env.local");
    if env_path.is_file() {
