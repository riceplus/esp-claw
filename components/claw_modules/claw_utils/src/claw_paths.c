...
{
    ESP_RETURN_ON_FALSE((unsigned)root < CLAW_PATH_ROOT_MAX,
                        ESP_ERR_INVALID_ARG, TAG, "bad root %d", (int)root);
    ESP_RETURN_ON_FALSE(path && path[0], ESP_ERR_INVALID_ARG, TAG, "empty path");
    ESP_RETURN_ON_FALSE(strlcpy(s_root_dirs[root], path, sizeof(s_root_dirs[root])) <
                            sizeof(s_root_dirs[root]),
                          ...
                          size_t out_size)
{
    ...
    ESP_RETURN_ON_FALSE(written >= 0 && (size_t)written < out_size,
                        ESP_ERR_INVALID_SIZE, TAG, "joined path too long: %s/%s",
