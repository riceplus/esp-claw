...
static esp_err_t lua_lvgl_to_root_vfs_path(claw_path_root_t root, const char *path, char *out, size_t out_size)
{
    const char *rel = lua_lvgl_skip_drive_prefix(path);

    ESP_RETURN_ON_FALSE(rel && rel[0], ESP_ERR_INVALID_ARG, TAG, "font fs path is empty");
    ESP_RETURN_ON_FALSE(!lua_lvgl_path_has_parent_ref(rel), ESP_ERR_INVALID_ARG, TAG, "font fs path escapes root");
    return claw_paths_join(root, rel, out, out_size);
}

static esp_err_t lua_lvgl_read_font_file(const char *vfs_path, uint8_t **out_data, size_t *out_size)
{
    FILE *fp;
    long file_size;
    uint8_t *data;
    ...
    if (fseek(fp, 0, SEEK_END) != 0) {
        fclose(fp);
        ESP_LOGW(TAG, "default font size seek failed: %s", vfs_path);
        return ESP_FAIL;
    }
    file_size = ftell(fp);
    if (file_size <= 0) {
        fclose(fp);
        ESP_LOGW(TAG, "default font file is empty: %s", vfs_path);
        return ESP_ERR_INVALID_SIZE;
    }
    if (fseek(fp, 0, SEEK_SET) != 0) {
        fclose(fp);
        ESP_LOGW(TAG, "default font rewind failed: %s", vfs_path);
        return ESP_FAIL;
    }
    ...
    if (!data) {
        fclose(fp);
        ESP_LOGW(TAG, "default font allocation failed: %ld bytes", file_size);
    ...
    if (read_len != (size_t)file_size) {
        free(data);
        ESP_LOGW(TAG, "default font read failed: %s, read=%lu size=%ld", vfs_path, (unsigned long)read_len, file_size);
        return ESP_FAIL;
    }
...
static int lua_lvgl_font_load(lua_State *L)
{
    ...
    uint8_t *font_data = NULL;
    size_t font_data_size = 0;
    char rel_path[LUA_MODULE_LVGL_PATH_MAX];
    char vfs_path[LUA_MODULE_LVGL_PATH_MAX];
    lv_font_t *font;
    ...
    err = lua_lvgl_to_lv_fs_path(path, rel_path, sizeof(rel_path));
    if (err != ESP_OK) {
        lua_lvgl_unlock();
    ...
    err = lua_lvgl_to_root_vfs_path(CLAW_PATH_DATA, rel_path, vfs_path, sizeof(vfs_path));
    if (err != ESP_OK) {
        lua_lvgl_unlock();
        return lua_lvgl_error_esp(L, "font data path", err);
    }
    data_ret = lua_lvgl_read_font_file(vfs_path, &font_data, &font_data_size);
    if (data_ret == ESP_ERR_NOT_FOUND) {
        err = lua_lvgl_to_root_vfs_path(CLAW_PATH_SYSTEM, rel_path, vfs_path, sizeof(vfs_path));
        if (err != ESP_OK) {
            lua_lvgl_unlock();
            return lua_lvgl_error_esp(L, "font system path", err);
        }
        data_ret = lua_lvgl_read_font_file(vfs_path, &font_data, &font_data_size);
    }
    if (data_ret != ESP_OK) {
        lua_lvgl_unlock();
        return luaL_error(L, "lvgl failed to load font: %s (%s)", rel_path, esp_err_to_name(data_ret));
    }
    ...
    if (!font) {
        free(font_data);
        lua_lvgl_unlock();
        return luaL_error(L, "lvgl failed to create font: %s size=%d", rel_path, size);
    }
    ...
    record->font = font;
    record->data = font_data;
    record->data_size = font_data_size;
