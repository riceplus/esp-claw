...
<Steps>
1. **Init Event Router and capabilities**

    `claw_event_router_init` loads `/fatfs/router_rules/router_rules.json`; `app_capabilities_init`
    then registers and starts the enabled C capabilities, including the system-only `agent` RPC adapter.
    Both unmatched-message-to-Agent and unmatched-Agent-output-to-channel fallbacks are enabled.
    Agent input is `call_cap("agent", method + args)`; Session output returns as `out_message` events.
