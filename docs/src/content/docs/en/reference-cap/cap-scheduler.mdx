...
This "separation of scheduling and execution" design allows scheduled tasks to reuse all Event Router actions (`call_cap`, `run_script`, `send_message`, etc.). Agent work uses `call_cap` with `cap="agent"`.
