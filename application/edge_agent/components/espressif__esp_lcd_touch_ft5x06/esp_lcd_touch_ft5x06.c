...
static esp_err_t touch_ft5x06_init(esp_lcd_touch_handle_t tp)
{
    ...
    /* Read chip info */
    {
        uint8_t chip_id = 0, firm_id = 0, lib_ver_h = 0, lib_ver_l = 0;
        touch_ft5x06_i2c_read(tp, 0xA8, &chip_id, 1);
        touch_ft5x06_i2c_read(tp, 0xA6, &firm_id, 1);
        touch_ft5x06_i2c_read(tp, 0xA1, &lib_ver_h, 1);
        touch_ft5x06_i2c_read(tp, 0xA2, &lib_ver_l, 1);
        ESP_LOGI(TAG, "chip_id=0x%02x firm_id=0x%02x lib_ver=%d.%d",
                 chip_id, firm_id, lib_ver_h, lib_ver_l);
    }
