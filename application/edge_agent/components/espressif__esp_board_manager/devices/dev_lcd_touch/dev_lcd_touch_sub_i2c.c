...
int dev_lcd_touch_sub_i2c_init(void *cfg, int cfg_size, void **device_handle)
{
    ...
    ESP_LOGI(TAG, "Got I2C bus handle: %p", i2c_bus_handle);
    i2c_refd = true;
    ...
    for (size_t i = 0; i < touch_cfg->sub_cfg.i2c.i2c_addr_count; i++) {
        ...
        ESP_LOGI(TAG, "Probing I2C addr 0x%02x (7-bit) ...", candidate_runtime_addr);
        ret = i2c_master_probe((i2c_master_bus_handle_t)i2c_bus_handle, candidate_runtime_addr, 200);
        ESP_LOGI(TAG, "Probe result: %s", esp_err_to_name(ret));
        if (ret == ESP_OK) {
    ...
    if (effective_addr == 0) {
        ESP_LOGE(TAG, "No LCD touch found on I2C bus: %s. Check wiring and I2C pull-ups.", touch_cfg->sub_cfg.i2c.i2c_name);
        goto init_err;
    }
    ESP_LOGI(TAG, "I2C probe successful at addr 0x%02x", effective_addr >> 1);
